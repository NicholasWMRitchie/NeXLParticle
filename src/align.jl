...
function calculate_score(data, mask, origin, radius, nrings, nθ, sx, sy)
    ...
    #       for r in 1:nrings ) #
    #           for off in 1:nθ ] )
    return findmax( [ sum( dot(circshift(mask, off), crd) for r in 1:nrings ) for off in 1:nθ ] )
end
...
function rough_align(data, datap, sx = :x, sy = :y, nrings=8, nθ=16)::AlignIntermediary
    ...
    scores = zeros(Float64, (length(xst), length(yst)))
    angles = zeros(Int, (length(xst), length(yst)))
...
if align_example
    ...
    spy(ai.scores)
    [ coordinate(ai, bb) for bb in findbest(ai) ]
end