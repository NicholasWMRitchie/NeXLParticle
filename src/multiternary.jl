...
function multiternary(
    ...
    function ternary(ctx, data, cola, colb, v1, L, θ)
        ...
        for lvl in range
            ...
            if length(pts)>0
                ...
                push!(datapts, ( ctx, Compose.circle(pa, pb, [L*0.01]),
                    Compose.stroke(col), Compose.fill("transparent"), Compose.clip([v3,v2,v1]) ))
                if !ismissing(clscol)
                    push!(datapts, (ctx, Compose.text(0.02, 0.54+ilvl*0.03, "$lvl - $(length(pts))"), Compose.stroke(col), Compose.fill(col)))
                end
    ...
    function label(ctx, v0, labels, L)
        ...
        return (ctx, Compose.text(xl, yl, ll, [hcenter], [vcenter]), Compose.stroke("black") )
    end
    ...
    if !ismissing(title)
        append!(ops, ( Compose.text(0.5, 0.03, title, hcenter, vtop), Compose.stroke("black")) )
    end
